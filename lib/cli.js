...
function runCopilot(argv) {
  ...
  if (scenarios.length == 1) {
    return runScenario(_.first(scenarios), options);
  }

  if (result.args.length == 2 && result.args[0] == 'run') {

    var scenario = result.args[1];
    if (scenario.match(/\.scenario\.js$/) && fs.existsSync(scenario)) {
      return runScenario(path.resolve(scenario), options);
    }
    ...
    return console.warn('Unknown API scenario ' + scenario);
  }
