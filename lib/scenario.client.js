...
module.exports = {
  ...
  startNextRequest: function() {
    ...
    if (cooldown && this.lastRequestResponseTime && now - this.lastRequestResponseTime < cooldown) {
      if (!this.requestCooldownTimeout) {
        this.requestCooldownTimeout = setTimeout(_.bind(this.startNextRequest, this), delay - (now - this.lastRequestResponseTime));
      }
      return;
    }
    ...
    var promise = this.startRequest(options).fin(_.bind(function() {
      ...
      this.requestCooldownTimeout = setTimeout(_.bind(this.startNextRequest, this), cooldown);
