...
var pkg = require('../package'),
    ...
    OPTIONS = [
      ...
      'requestPipeline', 'requestDelay', 'requestCooldown'
    ],
    PARAM_REGEXP = /^([^=]+)=(.*)$/;
  ...
  _.extend(Command.prototype, {
    ...
    parseCommandLineOptions: function(argv) {
      ...
      command
        ...
        .option('--request-pipeline <n>', 'maximum number of HTTP requests to run in parallel (no limit by default)', parseInt)
        .option('--request-delay <ms>', 'number of milliseconds to wait after starting an HTTP request to start another one (no delay by default)', parseInt)
        .option('--request-cooldown <ms>', 'number of milliseconds to wait after the end of the last completed HTTP request before starting another one (no cooldown by default)', parseInt);
