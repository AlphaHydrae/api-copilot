...
module.exports = function(deps) {
  ...
  _.extend(Scenario.prototype, {
    ...
    //     });
    //
    // By default, throws an error if the parameter was not found.
    // Set the `required` option to false to disable this behavior.
    //
    //     scenario.step('another step', function() {
    //       this.param('unknown', { required: false }); // undefined
    //     });
    ...
    // <a name="method-requireParameters"></a>
    // ### #requireParameters(names...)
    // Marks parameters as required.
    //
    // Use this method before defining steps to require that some parameters
    // be specified to run this scenario. If they are not available when running
    ...
    // ### #ensureParameters()
    // Throws an error if any of the required parameters specified with `#requireParameters`
    // were not specified.
    ...
    run: function(options) {
      ...
      // with runtime options. Runtime options are handled in
      // [cli.command.js](cli.command.js.html).
      ...
      // **EVENT:** emit the `configure` event with the run options.
      this.configure(runOptions);

      // **EVENT:** emit the `scenario:start` event with the run options.
      ...
      // Return a promise that will be resolved when the last step
    ...
    runStep: function(step, deferred) {
      ...
      // Build the step description object and extract the step arguments.
      // The step description object contains the **name** of the step.
      var description = { name: step.name },
          stepArgs = slice.call(arguments, 2);

      // **EVENT:** emit the `step:start` event with the step description object.
      this.emit.apply(this, [ 'step:start', description ].concat(stepArgs));

      // Call the step definition function with this scenario as the context
      ...
      // Call `#handleStepResult` if the promise is resolved, `#handleStepError` otherwise.
    ...
    handleStepResult: function(deferred, description) {
      ...
      // Extract the results from the previous step.
      // If it is a `StepArgs` object (such as returned by `#success`),
      // the results are spread over the arguments of the next step's definition function.
      ...
      if (this.stepSkipped) {
        // **EVENT:** emit the `step:skip` event if the step was skipped.
        ...
        // **EVENT:** emit the `step:done` event otherwise.
      ...
      // Stop the scenario if there are no more steps to run.
      if (!nextStep) {

        // **EVENT:** emit the `scenario:end` event.
        this.emit('scenario:end');

        return deferred.resolve();
      }

      // Run the next step with the results from the previous step as arguments.
    ...
    handleStepError: function(deferred, description, err) {
      ...
      // **EVENT:** emit the `step:error` event with the step description object and the error message.
      this.emit('step:error', description, err);

      // **EVENT:** emit the `scenario:error` event with the error message.
      this.emit('scenario:error', err);

      deferred.reject(err);
    }

  // Include [HTTP client extensions](scenario.client.js.html).
