...
module.exports = function(cliInfo, cliListing, cliRunner, cliEnv, fs) {
  ...
  _.extend(Program.prototype, {
    ...
    executeAction: function(name, command, arity, deferred) {
      ...
      var configFile = command.config || envOptions.config,
          config = this.loadConfigurationFileOptions(configFile);

      var options = _.extend({}, this.defaultOptions, this.pickOptions(config));
      options = merge(options, envOptions);
      options = merge(options, this.pickOptions(command, 'config'));
    ...
    loadConfigurationFileOptions: function(file) {

      var configFile = path.resolve(process.cwd(), file || 'api-copilot.yml');
    ...
    loadEnvironmentOptions: function() {
      ...
      var names = OPTIONS.concat([ 'config' ]);
