...
module.exports = function(cliInfo, cliListing, cliRunner, cliEnv, fs) {
  ...
  _.extend(Program.prototype, {
    ...
    loadConfigurationFileOptions: function(file, options) {
      file = path.resolve(file);

      if (fs.existsSync(file)) {

        var configFileOptions = yaml.safeLoad(fs.readFileSync(file, { encoding: 'utf8' }));
        if (!_.isObject(configFileOptions)) { // FIXME: this should cause an error
          return options;
        }
