...
module.exports = function(cliInfo, cliListing, cliRunner, cliEnv, fs) {
  ...
  _.extend(Program.prototype, {
    ...
    executeAction: function(name, command, arity, deferred) {
      ...
      if (!command.config.length || defaultConfigs) {
      ...
      if (configFiles) {
        _.each(configFiles, function(file) {
          promise = promise.then(_.bind(this.loadConfigurationFileOptions, this, file));
        }, this);
      }
    ...
    loadConfigurationFileOptions: function(file, options, required) {
    ...
    readConfigurationFile: function(file, required) {
      return q.nfcall(_.bind(fs.exists, fs), file).then(function(exists) {
        if (!exists) { // TODO: spec this
          return required ? q.reject(new Error('No such configuration file ' + file)) : undefined;
        }

        return q.nfcall(_.bind(fs.readFile, fs), file, { encoding: 'utf8' }).then(_.bind(yaml.safeLoad, yaml));
      });
    },
