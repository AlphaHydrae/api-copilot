  ...
  function showScenarioInfo(scenario, options) {
    ...
    if (scenario.parameters.length) {
      ...
      _.each(scenario.parameters, function(parameter) {
        print();
        print(parameter.describe().replace(/^/gm, '  ').replace(/^  /, '  -p '));
    ...
    if (options.config) {
      ...
      var loadedFiles = _.filter(options.config, fs.existsSync, fs);
      if (loadedFiles.length >= 2) {
        print('  Configuration files:');
        _.each(loadedFiles, function(file) {
          print('  - ' + path.resolve(file));
        });
      } else if (loadedFiles.length) {
        print('  Configuration file: ' + path.resolve(loadedFiles[0]));
      } else {
